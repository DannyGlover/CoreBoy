...
namespace CoreBoy.CpuOps
{
	...
	{
		public u8 Rl8(u8 inVal, bool checkZero, int cycles)
		{
			u8 carry = _gameboy.Flags.Get(Flags.C);
			u8 result = (u8)((inVal << 1) | (carry));
		...
		public void Rl8Mem(u16 address, bool checkZero, int cycles)
		{
			u8 data = _gameboy.Memory.ReadByte(address);

			data = Rl8(data, checkZero, cycles);
