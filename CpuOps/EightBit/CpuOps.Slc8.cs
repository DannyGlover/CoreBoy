...
namespace CoreBoy.CpuOps
{
	...
	{
		public u8 Slc8(u8 inVal, int cycles)
		{
			...
			if (_gameboy.Bit.Get(inVal, 7) == 0x1) _gameboy.Flags.Set(Flags.C);

			_gameboy.Cpu.Cycles += cycles;
			return result;
		}

		public void Slc8Mem(u16 address, int cycles)
		{
			u8 data = _gameboy.Memory.ReadByte(address);

			data = Slc8(data, cycles);
