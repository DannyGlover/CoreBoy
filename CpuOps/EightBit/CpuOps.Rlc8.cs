...
namespace CoreBoy.CpuOps
{
	...
	{
		public u8 Rlc8(u8 inVal, bool checkZero, int cycles)
		{
			u8 result = (u8)((inVal << 1) | (inVal >> 7));

			_gameboy.Flags.Clear(Flags.All);

			if (checkZero && (result == 0)) _gameboy.Flags.Set(Flags.Z);
			if (_gameboy.Bit.Get(inVal, 7) == 1) _gameboy.Flags.Set(Flags.C);

			_gameboy.Cpu.Cycles += cycles;
			return result;
		}

		public void Rlc8Mem(u16 address, bool checkZero, int cycles)
		{
			u8 data = _gameboy.Memory.ReadByte(address);

			data = Rlc8(data, checkZero, cycles);
