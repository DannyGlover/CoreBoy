...
namespace CoreBoy.CpuOps
{
	...
	{
		public u8 Inc8(u8 inVal, int cycles)
		{
			u8 result = (u8)(inVal + 1);

			_gameboy.Flags.Clear(Flags.Z | Flags.N | Flags.H);

			if (result == 0) _gameboy.Flags.Set(Flags.Z);
			if ((result & 0xF) == 0) _gameboy.Flags.Set(Flags.H);

			_gameboy.Cpu.Cycles += cycles;
			return result;
		}

		public void Inc8Mem(u16 address, int cycles)
		{
			u8 data = _gameboy.Memory.ReadByte(address);

			data = Inc8(data, cycles);
